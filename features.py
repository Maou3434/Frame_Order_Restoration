...
def extract_features(frames_folder, video_name, use_histogram=False, use_ncc=False):
    start_time = time.time()
    
    # Filter out non-image files (like meta.json)
    ...
    for idx, frame_path in enumerate(frame_paths):
        ...
        # Local refinement features (for comparisons, stored as reference)
        frame_features['mse_ref'] = frame_path  # store path for MSE computation later
        if use_ncc:
            frame_features['ncc_ref'] = frame_path  # store path for NCC computation later
...
if __name__ == "__main__":
    ...
    parser.add_argument('--frames_root_dir', type=str, required=True, help="Path to the root directory containing video frame subfolders (e.g., 'frames')")
    parser.add_argument('--video_name', type=str, required=True, help="Name of the video (e.g., 'jumbled_video'), which is also the name of the subfolder within frames_root_dir")
    ...
    # Construct the full path to the specific video's frames directory
    full_frames_path = os.path.join(args.frames_root_dir, args.video_name)

    extract_features(full_frames_path, args.video_name, args.use_histogram, args.use_ncc)
